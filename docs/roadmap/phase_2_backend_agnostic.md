...
- [ ] `bench/medium.yaml` workload descriptor — 100 snapshots/min, 1 000 rows/snapshot, 5 consumers, mix of webhook + Kafka sinks, 30-min CI run (full overnight job runs longer).
- [ ] CI matrix runs `medium` against each backend; numbers committed to `bench/results/<backend>/medium-<sha>.json`. The append-only history makes per-backend regressions visible.
