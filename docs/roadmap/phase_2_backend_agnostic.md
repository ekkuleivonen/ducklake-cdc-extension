...
The cross-product test for deferral 3 surfaced two pre-existing Stage-1 quirks worth chasing in their own follow-up — neither is correctness-breaking for the column-diff payload, but both confuse downstream consumers building per-table identity tracking. Pinning them here so the Phase 2 reviewer sees them.

- [ ] **Dual altered emission at rename snapshots.** When a transaction does `RENAME + column-list change` together (or DuckLake emits both `tables_created:"main"."new_name"` and `tables_altered:<id>` in one snapshot), Stage-1 emits two `altered.table` rows — one via the Finding-1 rename path with the new name and the cross-product details, and one via the ID-resolved path with the old name and the column-only details. They are not duplicates (the payloads differ) but a downstream consumer keying on `(snapshot_id, object_kind, object_id)` will see two rows for the same table at the same snapshot. Decision: deduplicate at the dispatcher (drop the ID-resolved row when a Finding-1 row exists for the same `(snapshot_id, table_id)`), or accept both and document a downstream `DISTINCT ON` recipe.
- [ ] **`LookupTableById` returns post-rename name from "wrong" side.** For ID-resolved tokens (`tables_altered:<id>`, `tables_dropped:<id>`), the lookup uses `WHERE table_id = ? LIMIT 1` with no snapshot bound — so it returns the *original* table_name after a rename rather than the name as-of the snapshot. This is what causes the `object_name='orders'` row to appear after a `RENAME TO orders_v2`. Fix: thread the snapshot bound through and use the same `(begin_snapshot <= snap AND end_snapshot IS NULL OR end_snapshot > snap)` predicate `LookupTableColumnsAt` already uses.
