...
| [1 — Extension MVP](./phase_1_extension_mvp.md) | Primitives + `cdc_consumer_*` lifecycle + **owner-token lease for single-reader enforcement** + the 60-second SQL demo + the matching README quickstart, against the embedded DuckDB catalog | **in progress** (core extension, docs, tests, and CI are present; examples, benchmark harness, release artifacts, and demo asset remain open) |
...
6. **We publish honest benchmark numbers, not hero numbers.** Three benchmark workloads (light / medium / heavy, ratified in Phase 0) are the intended release measurement set. The clean-slate tree has not restored the harness yet, so `docs/performance.md` currently records the targets and publication policy rather than measured history.
