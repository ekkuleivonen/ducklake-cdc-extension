...
| [2 — Catalog matrix, DDL coverage, DLQ, doctor](./phase_2_backend_agnostic.md) | Identical semantics on DuckDB / SQLite / Postgres catalogs (DuckLake's three supported metadata backends); `cdc_schema_diff` exhaustively tested; DLQ with **DDL-blocks-DML semantics**; `variant` / `geometry` serialization conventions; **doctor command** (moved up from Phase 5 so it's usable during testing) | **in progress** (catalog matrix smoke harness shipped on `feat_phase_02`; runtime-installed-DuckLake schema-evolution path is the open follow-up before catalog matrix integrates into CI) |
