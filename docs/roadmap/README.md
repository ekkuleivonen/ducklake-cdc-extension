...
| [1 — Extension MVP](./phase_1_extension_mvp.md) | Primitives + `cdc_consumer_*` lifecycle + **owner-token lease for single-reader enforcement** + the SQL walkthrough + the matching README quickstart, against the embedded DuckDB catalog | **complete** (closed 2026-04-30; `v0.0.0` cut against the ADR 0013 release pipeline; sanitiser pass landed as a release-time gate; three engineering items deferred to Phase 2 with rationale; visible-cadence release-tag discipline demoted to Phase 5) |
| [2 — Catalog matrix, DDL coverage, DLQ, doctor](./phase_2_backend_agnostic.md) | Identical semantics on DuckDB / SQLite / Postgres catalogs (DuckLake's three supported metadata backends); `cdc_schema_diff` exhaustively tested; DLQ with **DDL-blocks-DML semantics**; `variant` / `geometry` serialization conventions; **doctor command** (moved up from Phase 5 so it's usable during testing) | **in progress** (catalog matrix smoke wired into CI: DuckDB / SQLite / Postgres backends all green on every PR; the runtime-installed-DuckLake schema-evolution path was a Bind-time probe-snapshot bug in `cdc_changes`, fixed by mirroring `ReadWindow`'s end-snapshot computation read-only at Bind time) |
