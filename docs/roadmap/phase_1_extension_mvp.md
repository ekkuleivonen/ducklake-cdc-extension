...
Numbers without history are flat; numbers with history are a story. Phase 1 ships the first fast benchmark harness so maintainers can prove the CDC path is measurable before later phases add longer runs.

- [x] **Harness skeleton** in `bench/`:
  - `bench/runner.py` Python harness: seeds a DuckLake with a controllable producer, runs a consumer with workload parameters `(load_profile, duration_seconds, target_snapshots_per_second, target_rows_per_snapshot, consumers, max_snapshots)`, captures **end-to-end latency p50/p95/p99/max/mean** (via a monotonic-time ledger so cross-process clock skew is irrelevant), **consumed events/sec**, **catalog QPS**, **operation timings** (`producer_insert`, `cdc_window`, `cdc_changes`, `cdc_commit`), and **lag drift over the run**. Consumer CPU/RSS is deferred to a future revision — we get the headline metrics this phase needs without a `psutil` dependency. The runner's shape must make load profiles easy to add later; Phase 1 only implements `flat`.
  - `bench/light.yaml` workload descriptor — a fast smoke benchmark, not a soak: 60-second flat profile, 0.5 target snapshots/sec, 100 target rows/snapshot, 1 consumer. The manual benchmark workflow uses this descriptor as-is; longer runs and variable-load profiles land after the harness has earned trust.
  - `bench/results/` — JSON result location with `(result_schema_version, run_id, timestamp_utc, commit, hardware_label, workload, measurements)`. CI uploads each result as an artifact; maintainers commit selected baselines when they want an auditable trajectory. Per ADR 0011: committed regressions are explained in commit messages, not deleted.
- [x] **Manual benchmark workflow (Phase 1, against light only) — soft, not a build break.**
  - **Hard requirement for manual runs:** the harness runs successfully and produces a number. Wired into `.github/workflows/benchmark.yml`, outside the required Full CI gate; the workflow downloads the `linux_amd64` extension artifact from a successful Full CI run and executes `uv run --locked python bench/runner.py --runtime official --cdc-extension ... --workload bench/light.yaml` against the supported official DuckDB release.
...
- [x] `bench/runner.py` + `bench/light.yaml` ship and run as a manual benchmark workflow against a Full-CI-built extension artifact and the supported official DuckDB release. CI uploads result JSON as an artifact; selected baselines can be committed to `bench/results/` once there is a trajectory worth preserving. **The harness is the deliverable** — bad numbers are acceptable in Phase 1 because we now have a way to show improvement.
