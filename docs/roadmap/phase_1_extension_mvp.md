...
- [x] **Harness skeleton** in `bench/`:
  ...
  - `bench/light.yaml` workload descriptor — a fast smoke benchmark, not a soak: 60 seconds, 30 snapshots/min, 100 rows/snapshot, 1 consumer. The manual benchmark workflow uses this descriptor as-is; longer runs and variable-load profiles land after the harness has earned trust.
