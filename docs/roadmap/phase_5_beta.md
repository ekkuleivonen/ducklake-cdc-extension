...
- [ ] **Heavy benchmark workload (`bench/heavy.yaml`) ratified in ADR 0011 lands in this phase.** 1 000 snapshots/min, 10 000 rows/snapshot, 20 consumers, Kafka sink. Targets: p99 latency < 30s, throughput matches producer at steady state. Goes into the `bench/results/` history alongside light (since Phase 1) and medium (since Phase 2).
