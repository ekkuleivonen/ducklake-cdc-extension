...
Usage:
    uv run python e2e/upstream/enumerate_changes_map.py --backends duckdb sqlite [postgres]
    uv run python e2e/upstream/enumerate_changes_map.py --backends duckdb sqlite [postgres] --check
    uv run python e2e/upstream/enumerate_changes_map.py --update-docs
...
Exit codes:
    ...
    3 = current key sets diverged from committed e2e/upstream/output/ reference
...
def _format_reference_drift(deltas: dict[str, dict[str, dict[str, list[str]]]]) -> str:
    ...
    out.extend(
        [
            ...
            "      uv run python e2e/upstream/enumerate_changes_map.py \\",
            "          --backends duckdb sqlite postgres --update-docs",
            "    locally, review the resulting diff, and commit the regenerated",
            "    e2e/upstream/output/ files plus the docs/api.md splice. The DDL /",
...
def _render_reference_markdown(results: dict[str, dict[str, Any]]) -> str:
    ...
    out: list[str] = []
    out.append("<!-- Generated by e2e/upstream/enumerate_changes_map.py — do not edit by hand. -->")
    out.append("")
    ...
    if REFERENCE_BEGIN in text and REFERENCE_END in text:
        ...
        if anchor in text:
            new_text = text.replace(
                "_To be populated by the upstream probe output in `e2e/upstream/enumerate_changes_map.py`",
                block + "\n\n_To be populated by the upstream probe output in `e2e/upstream/enumerate_changes_map.py`",
                1,
            )
        else:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--check",
        action="store_true",
        help="Compare current MAP-key projection against committed e2e/upstream/output/ JSON.",
    )
    parser.add_argument(
        "--keep-workdir",
        action="store_true",
