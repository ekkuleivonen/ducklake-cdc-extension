...
Primary connections use :mod:`ducklake_client` for attach; the secondary handle
replays extension loads to match the historical multiconnection harness.
...
DEFAULT_POSTGRES_DSN = "host=127.0.0.1 port=5436 user=ducklake password=ducklake dbname=ducklake"
...
def _duckdb_config_for_backend(name: str) -> DuckDBConfig:
    cfg: dict[str, str | bool | int] = {"allow_unsigned_extensions": True}
    if name == "postgres":
        cfg["pg_pool_max_connections"] = 64
        cfg["pg_connection_limit"] = 64
    return DuckDBConfig(config=cfg)


@contextmanager
def attached_pair(backend: Backend) -> Iterator[tuple[duckdb.DuckDBPyConnection, duckdb.DuckDBPyConnection]]:
    lake = DuckLake(
        catalog=backend.catalog,
        storage=backend.storage,
        alias="lake",
        duckdb=_duckdb_config_for_backend(backend.name),
    )
    b: duckdb.DuckDBPyConnection | None = None
    try:
        a = lake.connection
        a.load_extension(str(CDC_EXTENSION))
...
def duckdb_backend(workdir: Path, _: str) -> Backend:
    ...
    return Backend(
        name="duckdb",
        catalog=DuckDBCatalog(path=catalog),
        storage=DiskStorage(path=data),
...
def sqlite_backend(workdir: Path, _: str) -> Backend:
    ...
    return Backend(
        name="sqlite",
        catalog=SqliteCatalog(path=catalog),
        storage=DiskStorage(path=data),
...
def postgres_backend(workdir: Path, dsn: str) -> Backend:
    ...
    return Backend(
        name="postgres",
        catalog=PostgresCatalog(dsn=dsn),
        storage=DiskStorage(path=data),
        before_attach=(
            "INSTALL postgres",
            "LOAD postgres",
