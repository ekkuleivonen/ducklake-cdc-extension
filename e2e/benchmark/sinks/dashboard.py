...
- :class:`DemoSink` — a thin sink that forwards each
...
The dashboard is observability-only (``required=False``). Render
...
class DemoDashboard:
    ...
    def start(self) -> None:
        ...
        # was previously installed — typically the benchmark app's flag-only
    ...
    def stop(self) -> None:
        ...
        # this point — the benchmark app has already restored too).
    ...
    def _handle_signal(self, signum: int, frame: FrameType | None) -> None:
        ...
        frozen in alt-screen mode while the benchmark app drains
...
class DemoSink:
    ...
    per-table consumer. ``required=False`` so dashboard hiccups never
    gate delivery.
    """
