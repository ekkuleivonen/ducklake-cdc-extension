"""Generate DuckLake changes for the CDC benchmark consumer."""
...
from ducklake_client import DuckLake, DuckLakeError
...
class SqlRunner(Protocol):
    def sql_dicts(self, query: str, **named_parameters: object) -> list[dict[str, Any]]: ...
...
def retry_sql(lake: DuckLake, query: str) -> None:
    retry_on_lock(lambda: lake.sql_dicts(query))
...
def apply_action(lake: SqlRunner, action: Action, args: Args) -> None:
    if action.kind == "insert":
        ...
        lake.sql_dicts(f"DELETE FROM {action.table.qualified} WHERE id = $id", id=action.row_id)
    else:
