...
exact wording. The complementary SQL test `test/compat_check.test`
covers the silent paths (no DuckLake attached, compatible DuckLake
attached); this script covers the warning/error text path.

What this script does (no Docker; stdlib + repo paths via `_harness_env`):
...
from _harness_env import BUILD, DUCKDB_CLI  # noqa: E402
